...
├── docs/
│   └── endpoints/              # Endpoint documentation (one .md per endpoint group)
├── hoppscotch/                 # API testing collections
...
Create a markdown file in `docs/endpoints/` for each endpoint group. Follow this format:
...
|-----------|------|---------|-------------|
| week_offset | int | 0 | Week offset (0 = current, -1 = last week) |
...
Include Hoppscotch JSON collection files in `hoppscotch/` for testing all API endpoints:

```
hoppscotch/
├── CLAUDE.md           # Instructions for using the collection
└── collection.json     # Exported Hoppscotch collection
```

Update the collection whenever new endpoints are added.
