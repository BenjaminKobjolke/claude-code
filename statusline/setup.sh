...
do_install() {
  ...
  echo ""
  echo "  Tip: The rate limit widget calls the API"
  echo "  every 3 min (~340ms on cache miss, cached"
  echo "  reads are instant). Set SHOW_RATELIMIT=0"
  echo "  in statusline.conf to skip it entirely."
  echo "  Non-USD locales fetch exchange rates once"
  echo "  per week in the background (~200ms)."
  echo ""
