...
1. List all `.md` files in the `plan/` directory, excluding any files inside `plan/done/`. If no plan files exist, tell the user there are no plans to implement and suggest they create one with `/xida-tools:plan-feature`.
...
7. **On failure**: If any step fails or you encounter an unexpected problem:
   ...
   - Ask the user if they want to attempt a fix or create a handoff via `/xida-tools:handoff-create`
   - Do NOT move the plan to `done/` if implementation is incomplete

8. After all implementation steps are complete, run `/xida-tools:validate-pre-commit`. If validation fails, fix the issues and re-run validation until it passes.
...
10. Tell the user the implementation is complete and suggest they review the changes and commit with `/xida-tools:git-commit`. Do NOT auto-commit.
